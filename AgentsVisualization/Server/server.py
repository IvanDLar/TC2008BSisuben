...
def getAgents():
    ...
    if request.method == 'GET':
        agentPositions = [{"id": str(a.unique_id), "x": x, "y": 0.08, "z":z} for (a, x, z) in randomModel.grid.coord_iter() if isinstance(a, RandomAgent)]
...
def getBoxes():
    ...
    if request.method == 'GET':
        boxPositions = [{"id": str(a.unique_id), "x": x, "y":0.3, "z":z} for (a, x, z) in randomModel.grid.coord_iter() if isinstance(a, BoxAgent)]
